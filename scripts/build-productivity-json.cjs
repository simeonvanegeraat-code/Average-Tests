...
(function main(){
  const focus = loadCsv("productivity_focus_span_minutes_mean.csv");
  const coffee = loadCsv("productivity_coffee_cups_per_day_mean.csv");
  const study = loadCsv("productivity_study_hours_per_week_mean.csv");
  const items = [];
  pushSimple(focus, "focus_span", "Sustained focus span", "min", "focus_span_minutes_mean", "Lab studies/surveys (demo)", items);
  pushSimple(coffee, "coffee_cups", "Coffee per day", "cups", "coffee_cups_per_day_mean", "Food surveys (demo)", items);
  pushSimple(study, "study_hours", "Study hours per week", "h/wk", "study_hours_per_week_mean", "Education surveys (demo)", items);
  ["focus_span","coffee_cups","study_hours"].forEach(id=>globalize(items, id));
  fs.mkdirSync(path.dirname(OUT("productivity.json")), {recursive:true});
  fs.writeFileSync(OUT("productivity.json"), JSON.stringify(items, null, 2));
  console.log(`✔ Wrote ${OUT("productivity.json")} with ${items.length} records`);
})();
